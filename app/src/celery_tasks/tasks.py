...
def send_email(message):
    ...
    if "notify_due_date" in message:
        message_body = f'\n\nAnnouncement: todo with:\nTitle: {message["title"]}\nDescription: {message["description"]}is about to expire!'
    if "notify_morning" in message:
        message_body = f'\n\nAnnouncement at 8.00 AM:\The number of todos to complete today is: {message["todo_to_complete"]}\Amount of todos due but not completed is: {message["todo_overdue"]}!'
...
# celery - A celery_app worker - -loglevel = INFO - B để bật celery beat
...
def check_due_date():
    ...
    todos = db.query(models.ToDo).filter(
        models.ToDo.status == "Unfinish").all()
    for todo in todos:
        ...
        if time_diff <= 15 and time_diff > 0 and min_time >= 15:
            ...
            user = db.query(models.User).filter(
                models.User.id == list_todo.user_id).first()
            message = {
                "receiver": user.email,
                "title": todo.title,
...
def morning():
    ...
    for user in users:
        ...
        lists = db.query(models.ToDoList).filter(
            models.ToDoList.user_id == user.id).all()
        for list_todo in lists:
            todos = db.query(models.ToDo).filter(
                models.ToDo.status == "Unfinish", list_id=list_todo.id).all()
            for todo in todos:
                due_date = todo.due_date
                current_time = datetime.datetime.now().replace(second=0, microsecond=0)
                time_diff = (due_date - current_time).total_seconds() / 60.0
